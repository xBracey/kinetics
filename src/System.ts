...
export default class System extends EventEmitter {
  ...
  public get momentum() {
    ...
    for (const entity of this.entities) {
      if (!entity) continue;
      momentum +=
        entity.velocity.magnitude * entity.mass +
        Math.abs(entity.angularVelocity * entity.inertia) * entity.mass;
    }
  ...
  public get kineticEnergy() {
    ...
    for (const entity of this.entities) {
      ...
      energy +=
        0.5 *
        entity.mass *
        (entity.velocity.magnitude * entity.velocity.magnitude);
    }
  ...
  constructor(config: SystemConfig) {
    ...
    this.environment =
      typeof window === "undefined" ? Environment.Node : Environment.Browser;
    this.config = config;
    ...
    if ((this.width && !this.height) || (this.height && !this.width))
      throw new ConfigurationError(
        "Both dimensions must be specified for the system."
      );
    ...
    if (!config.collisionInfo)
      throw new ConfigurationError(
        "Collision information must be specified for the system."
      );

    this.CollisionManager = new SpatialHashGrid(
      this,
      config.collisionInfo.cellSize || 12
    );
