...
export default class CollisionResolver {
  ...
  public detectSimple(entity1: Entity, entity2: Entity): boolean {
    if (
      entity1.type === EntityType.Circle &&
      entity2.type === EntityType.Circle
    )
      return this.detectCircleCircle(entity1 as Circle, entity2 as Circle);
    if (
      entity1.type === EntityType.Circle ||
      entity2.type === EntityType.Circle
    ) {
    ...
    for (let i = 0; i < edges; i++) {
      ...
      if (i < vertices1.length)
        normal = vertices1[i].clone
          .subtract(vertices1[(i + 1) % vertices1.length])
          .orthogonal.normalize();
      else
        normal = vertices2[i - vertices1.length].clone
          .subtract(vertices2[(i + 1) % vertices2.length])
          .orthogonal.normalize();
  ...
  private detectCirclePolygon(circle: Circle, polygon: Entity) {
    ...
    for (let i = 0; i < vertices.length; i++) {
      ...
      const overlapN =
        Math.min(max, circleProjection + circle.radius) -
        Math.max(min, circleProjection - circle.radius);
  ...
  private resolve(
    ...
    const impulse =
      (-(1 + elasticity) * velocityProjection) / (1 / mass1 + 1 / mass2);
    ...
    if (!entity1.static && !entity2.static) {
      entity1.angularVelocity -=
        (1 / entity1.inertia) *
        entity1.position.clone.subtract(entity2.position).cross(impulseVector);
      entity2.angularVelocity -=
        (1 / entity2.inertia) *
        entity1.position.clone.subtract(entity2.position).cross(impulseVector);
    }
